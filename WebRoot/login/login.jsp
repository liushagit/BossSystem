...
<div class="container">
  <div class="content">
  <FORM name=theForm onsubmit="sub()" action=/login/index method=post>
     <div class="main">
         <img alt="用户登录" src="/admin/images/login_tit.gif" class="l_tit" />
         <div class="form">
            <p><label>账  号：</label><input name="login.username" value="${login.username}" id="username" type="text"/>${usernameMsg}</p>
            <p><label>密  码：</label><input name="login.password" value="${login.password}" id="password" type="password"/>${passwordMsg}</p>
            <p><label>验证码：</label><input name="login.captcha" value="${login.captcha}" id="captcha" type="text"/>${captchaMsg}
