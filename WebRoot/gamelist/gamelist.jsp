...
function exp1(){
	...
	window.location.href="/gamelist/exportExcel?startDate="+startDate+"&endDate="+endDate+"&appId="+appId+"&clquery="+clquery+"&channel="+channel+"";
}
</script>
...
<form action="/gamelist/index" method="post">
    开始日期&nbsp;
 ...
 游戏名称
	...
	<input type=button name="exp" value="生成Excel" onclick="exp1()" />
</div>
</form>
</tr>
		...
		<tr>
                    <!-- 左边用来显示分页的总参数-->
                        ...
                        <td width="80%" align=right vAlign="center" noWrap><a
                            href="/gamelist?startDate=${defaultStart }&endDate=${defaultEnd }&appId=${appId}&channel=${channeled }">首页</a>
                            <!-- 判断是不是第一页决定是否显示前一页标签 -->
                            <c:if test="${pages.pageNumber ne 1 }">
                            <a href="/gamelist/${pages.pageNumber - 1 }?startDate=${defaultStart }&endDate=${defaultEnd }&appId=${appId}&channel=${channeled }">上一页</a>
                            </c:if>
                            ...
                            <c:forEach begin="${begin }"
                                ...
                                <c:if test="${pages.pageNumber ne i }">
                                    <a href="/gamelist/${i }?startDate=${defaultStart }&endDate=${defaultEnd }&appId=${appId}&channel=${channeled }">${i }</a>
                                </c:if>
                            </c:forEach>
                            <!-- 判断是不是最后一页，决定是否显示后一页下标-->
                            <c:if test="${pages.pageNumber ne pages.totalPage }">
							<!-- 昨天没有测试出来，应该是pageNumber而不是totalPage-->
 							<a href="/gamelist/${pages.pageNumber +1}?startDate=${defaultStart }&endDate=${defaultEnd }&appId=${appId}&channel=${channeled }">下一页</a>
 							<!-- 
 							(<%=sdf1%>==null?startDate=${defaultStart}:startDate=<%=sdf1%>)&(<%=sdf2%>==null?endDate=${defaultEnd}:endDate=<%=sdf2%>)
 							 -->
                            </c:if>
                            <!-- 显示尾页下标 -->
                             <a href="/gamelist/${pages.totalPage }?startDate=${defaultStart }&endDate=${defaultEnd }&appId=${appId}&channel=${channeled }">尾页</a>
                    </td>
                    </tr>
		</table>   
      </DIV></TD>    
